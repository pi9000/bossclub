...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $banner = DB::table('tb_banner')->where('agent_id', general()->agent_id)->where('status', 'active')->get();
        $popup = DB::table('tb_popup')->where('agent_id', general()->agent_id)->where('status', 'active')->first();
        ...
        $promotion = DB::table('tb_bonus')->where('status', 'active')->where('agent_id', general()->agent_id)->get();
    ...
    {
        $user = DB::table('users')->where('username', $request->username)->where('agent_id', general()->agent_id)->first();
        if (!$user) {
            return response()->json([
    ...
    {
        $user = DB::table('users')->where('no_hp', $request->phone)->where('agent_id', general()->agent_id)->first();
        if (!$user) {
            return response()->json([
    ...
    {
        $bank = DB::table('users')->where('nomor_rekening', $request->optAccountNumber)->where('agent_id', general()->agent_id)->first();
        if (!empty($bank)) {
    ...
    {
        $bank = DB::table('users')->where('nomor_rekening', $request->norek)->where('agent_id', general()->agent_id)->first();
        if (!$bank) {
            return response()->json([
    ...
    public function transfer_money(Request $request)
    {
        ...
        if ($request->id_game == null && $request->password_game == null) {
...
<b>Date</b> : ' . date('Y-m-d H:i:s') . '';
            ...
            return redirect()->back()->with('success', __('public.create_id'));
        }


        if ($request->type == 1) {

            $check = DB::table('tb_transaksi')->where('id_user', auth()->user()->id)->where('transaksi', 'Top Up')->where('dari_bank', 'Main Balance')->where('agent_id', general()->agent_id)->where('status', 'Pending')->first();
            if (!empty($check)) {
...
<b>Date</b> : ' . date('Y-m-d H:i:s') . '';
            ...
            if ($result['ok']) {
                ...
                $trans->bonus = 'tanpabonus';
                $trans->bonus_amount = 0;
                $trans->keterangan = 'Withdraw Credit Games Account : ' . $request->id_game . ' To Main Wallet';
                $trans->status = 'Pending';
                $trans->id_user = auth()->user()->id;
                $trans->username = auth()->user()->username;
                $trans->save();
            }
            return back()->with('success', __('public.apk_withdraw_success'));
        }
    }
}
